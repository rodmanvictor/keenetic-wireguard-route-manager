...
Я сохраню только `chatgpt.com`, получу его текущие IPv4 и IPv6-адреса и добавлю
точные `/32` и `/128` маршруты на Keenetic. Через 6 часов проверю адреса снова.
...
- маршрутизация работает с IPv4 и IPv6; для IPv6 туннель должен иметь IPv6 `Address` и подходящие `AllowedIPs`;
